...
class ReadingDashboardPage extends StatefulWidget {
  const ReadingDashboardPage({super.key});

  @override
  State<ReadingDashboardPage> createState() => _ReadingDashboardPageState();
}

class _ReadingDashboardPageState extends State<ReadingDashboardPage> {
  ...
  void initState() {
    super.initState();
    context.read<ReadingCubit>().loadHistory();
  }
    ...
    cubit.analyzeText(
      ...
      onSuccess: () {
        ...
        if (state is ReadingLoaded && state.currentArticle != null) {
          ...
          context.push('/reading/article/$id');
        }
      },
    );
  }
      ...
      appBar: AppBar(
        title: const Row(
          children: [
            ...
            Text('长文阅读与语法解析', style: TextStyle(fontWeight: FontWeight.bold)),
          ],
        ),
            ...
            if (state is ReadingError) {
              return Padding(
                ...
                child: Column(
                  ...
                  children: [
                    const Text('加载长文库历史失败', style: TextStyle(fontSize: 16, color: Colors.redAccent, fontWeight: FontWeight.bold)),
                    const SizedBox(height: 8),
                    Text(state.message, style: const TextStyle(color: Colors.grey), textAlign: TextAlign.center),
                    const SizedBox(height: 24),
                    ElevatedButton(
                      onPressed: () => readingCubit.loadHistory(),
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Form(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          crossAxisAlignment: CrossAxisAlignment.stretch,
                          children: [
                            const Text('✏️ 上传分析新长文', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: Colors.white)),
                            ...
                            TextFormField(
                              ...
                              decoration: InputDecoration(
                                hintText: '例如: The Road Not Taken',
                                border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                              ),
                            ),
                            const SizedBox(height: 16),
                            const Text('英文内容', style: TextStyle(color: Colors.grey, fontSize: 12, fontWeight: FontWeight.bold)),
                            const SizedBox(height: 6),
                            TextFormField(
                              ...
                              decoration: InputDecoration(
                                hintText: '请粘贴或输入你想分析的英文长文（建议在5000字符以内）...',
                                border: OutlineInputBorder(borderRadius: BorderRadius.circular(12)),
                              ),
                            ),
                            if (error != null) ...[
                            ...
                            ElevatedButton(
                              ...
                              child: analyzing
                                  ...
                                  : const Text('🚀 开始深度阅读解析', style: TextStyle(fontSize: 15, fontWeight: FontWeight.bold, color: Colors.white)),
                            ),
                          ],
                        ),
                      ),
                    ...
                    Row(
                      ...
                      children: [
                        const Text('📚 个人长文库历史', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: Colors.white)),
                        TextButton(
                          onPressed: () => readingCubit.loadHistory(),
                      ...
                      ListView.builder(
                        ...
                        itemBuilder: (context, index) {
                          ...
                          return Padding(
                            ...
                            child: InkWell(
                              onTap: () => context.push('/reading/article/${art.id}'),
                              borderRadius: BorderRadius.circular(16),
                              child: Container(
