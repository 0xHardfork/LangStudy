...
func (s *service) saveDialogueAndAssets(ctx context.Context, cfg *llmconfig.LLMConfig, userID uint, language, level, topic string, llmLines []llmDialogueLine) (*Dialogue, error) {
	...
	for i, line := range d.Lines {
		...
		go func(i int, line DialogueLine) {
			...
			relPath := fmt.Sprintf("static/audio/%d/%d.mp3", d.ID, i)
			physicalPath := filepath.Join(s.staticDir, fmt.Sprintf("audio/%d/%d.mp3", d.ID, i))
			if ttsErr := generateAudio(ttsCtx, line.OriginalText, language, line.Speaker, physicalPath); ttsErr != nil {
