...
func main() {
	...
	if cfg.App.Env != "production" {
		log.Info("automatically running database migrations in dev environment...")
		if err := database.RunMigrations(sqlDB, migrations.FS, log); err != nil {
			log.Fatal("run migrations failed", zap.Error(err))
		}
	} else {
	...
	llmCli := llm.NewClient(time.Duration(cfg.LLM.TimeoutSeconds)*time.Second, log)
	...
	dialogueService := dialogue.NewService(dialogueStore, llmStore, log, cfg.App.StaticDir, llmCli)
