...
- **提示词可配置化要求 (Prompt Configuration)**：
  - 凡是涉及到大语言模型（LLM）的 Prompt，**严禁在代码中直接硬编码**。
  - 必须将其作为配置字段添加到管理端配置中（更新 `llm_configs` 表的模型定义和 `UpdateConfigRequest`），同时更新前端 `AdminDashboard` 页面，使其在管理端可以直接配置。
  - **必须同步在 `migrations/` 的 SQL 迁移脚本中，将初始默认的 Prompt 模板作为初始化数据写入数据库，确保全新部署后后台能够展示并加载默认模板。**
  - 代码中通过 `llmconfig.Service` 动态获取 Prompt，仅当获取到的配置值为空时，才允许使用硬编码的默认 Prompt 串进行降级兜底。
...
### 4. 验证与构建
- 在提交前运行本地编译与测试：
  - 后端：编译 `go build ./...` 并运行单元测试 `go test ./...`
  - 前端：`npm run build` (vite build & tsc 编译)
- 运行新增功能的对应测试，确保不带任何编译错误、测试未通过或崩溃问题交付。
