  ...
  const fetchCaptcha = async () => {
    try {
      ...
      if (json.code === 0 && json.data) {
        setCaptchaId(json.data.captcha_id)
        setCaptchaImg(json.data.captcha_image)
        setCaptchaCode('')
      }
    } catch (err) {
        ...
        <form onSubmit={handleSubmit} className="space-y-5">
          ...
          <div>
            ...
            <div className="flex gap-3">
              <input
                type="text"
                className="flex-1 rounded-lg border border-slate-800 bg-slate-950/50 px-4 py-2.5 text-sm text-white placeholder-slate-600 focus:border-blue-500 focus:outline-none transition-colors"
                placeholder="Code"
                value={captchaCode}
              ...
              {captchaImg && (
                <img
                  ...
                  className="h-10 w-28 rounded-lg border border-slate-800 cursor-pointer object-cover bg-white select-none hover:opacity-90 transition-opacity"
